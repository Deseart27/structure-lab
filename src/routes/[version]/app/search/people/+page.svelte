<script lang="ts">
	...
	// HubSpot push state per row
	const v6PushedToHS: Record<number, boolean> = { 0: true, 2: true, 6: true };
	let v6SessionPushed = $state(new Set<number>());

	function v6IsPushed(i: number): boolean {
		return v6PushedToHS[i] === true || v6SessionPushed.has(i);
	}

	function v6PushToHS(i: number, personName: string) {
		if (v6IsPushed(i)) return;
		const next = new Set(v6SessionPushed);
		next.add(i);
		v6SessionPushed = next;
		toast.show(`${personName} pushed to HubSpot`);
	}
...
<div class="flex h-full flex-col">
	...
	<div class="border-grey-200 flex h-14 shrink-0 items-center justify-between border-b px-6">
		...
		<div class="flex items-center gap-2">
			...
			{:else if version === 'v6' || version === 'v7' || version === 'v8' || version === 'v9' || version === 'v10' || version === 'v11' || version === 'v12'}
				...
				<tbody class="bg-white">
					...
					{@const rowLists = v6GetLists(i)}
						<tr class="border-grey-100 hover:bg-grey-50 border-b transition-colors" class:bg-violet-50={v6Selected.has(i)}>
								...
								{:else}
									<div class="relative">
										<button
											...
											onclick={() => { if (version === 'v9' || version === 'v10' || version === 'v11' || version === 'v12') { v9EnrichPopover = { index: i, type: 'email' }; v9EnrichList = 'none'; } else { v6EnrichRow(i); } }}
										>
											<span class="material-icons-round text-sm">mail</span>
											Find email
										</button>
										{#if (version === 'v9' || version === 'v10' || version === 'v11' || version === 'v12') && v9EnrichPopover?.index === i && v9EnrichPopover?.type === 'email'}
							...
							<td class="px-4 py-3 text-sm">
								...
								{:else}
									<div class="relative">
										<button
											class="flex h-7 items-center gap-1 rounded-lg border border-grey-200 bg-white px-2 text-xs font-medium text-grey-600 shadow-sm transition-colors hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700"
											onclick={() => { if (version === 'v9' || version === 'v10' || version === 'v11' || version === 'v12') { v9EnrichPopover = { index: i, type: 'phone' }; v9EnrichList = 'none'; } else { v6EnrichRow(i); } }}
										>
											<span class="material-icons-round text-sm">phone</span>
											Find phone
										</button>
										{#if (version === 'v9' || version === 'v10' || version === 'v11' || version === 'v12') && v9EnrichPopover?.index === i && v9EnrichPopover?.type === 'phone'}
											<button class="fixed inset-0 z-30" onclick={() => { v9EnrichPopover = null; }} aria-label="Close"></button>
											<div class="absolute left-0 top-full z-40 mt-1 w-64 rounded-xl border border-grey-200 bg-white p-4 shadow-xl">
												<div class="mb-3 flex items-center gap-2">
							...
							<td class="px-1 py-3">
								<button
									class="flex h-7 w-7 items-center justify-center rounded-lg transition-colors {v6IsPushed(i) ? 'opacity-100' : 'opacity-40 hover:opacity-70'}"
									title={v6IsPushed(i) ? 'Already in HubSpot' : 'Push to HubSpot'}
									onclick={() => v6PushToHS(i, person.name)}
								>
									<svg class="h-3.5 w-3.5" viewBox="0 0 1024 1024">
										<circle cx="512" cy="512" r="512" fill={v6IsPushed(i) ? '#FF7A59' : '#9CA3AF'}/>
										<path d="M623.8 624.94c-38.23 0-69.24-30.67-69.24-68.51s31-68.52 69.24-68.52 69.26 30.67 69.26 68.52-31 68.51-69.26 68.51m20.74-200.42v-61a46.83 46.83 0 0 0 27.33-42.29v-1.41c0-25.78-21.32-46.86-47.35-46.86h-1.43c-26 0-47.35 21.09-47.35 46.86v1.41a46.85 46.85 0 0 0 27.33 42.29v61a135.08 135.08 0 0 0-63.86 27.79l-169.1-130.17A52.49 52.49 0 0 0 372 309c0-29.21-23.89-52.92-53.4-53s-53.45 23.59-53.48 52.81 23.85 52.88 53.36 52.93a53.29 53.29 0 0 0 26.33-7.09l166.38 128.1a132.14 132.14 0 0 0 2.07 150.3l-50.62 50.1A43.42 43.42 0 1 0 450.1 768c24.24 0 43.9-19.46 43.9-43.45a42.24 42.24 0 0 0-2-12.42l50-49.52a135.28 135.28 0 0 0 81.8 27.47c74.61 0 135.06-59.83 135.06-133.65 0-66.82-49.62-122-114.33-131.91" fill="#fff" fill-rule="evenodd"/>
									</svg>
								</button>
							</td>
